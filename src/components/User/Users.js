          ...
          sendRequest(
            ...
            (scheduleResponse) => {
              const currentSchedule =
                scheduleResponse?.MRData?.RaceTable?.Races;
        ...
        dispatch(
          fetchScheduleFailure(
            errorSchedule?.message ||
              errorSchedule ||
              "Someting went wrong! Try again later."
          )
        );
      }
    };
      ...
      try {
        if (!drivers?.length) {
          ...
          sendRequest(
            ...
            (dataSet) => {
              const driversArray =
                dataSet?.MRData?.StandingsTable?.StandingsLists[0]
                  ?.DriverStandings;
                  ...
                  drivers?.map((driver, index) => {
                    if (
                      ...
                      const driverProfilePic = driversScheduleImages.find(
                        (driverImg) =>
                          driver?.Driver?.driverId === driverImg?.driverId
                      );
                      return (
                        <DriverCard
                          ...
                          driverNationalityFlag={
                            driverProfilePic?.nationalityFlag
                          }
                        />
                      );
                    }
