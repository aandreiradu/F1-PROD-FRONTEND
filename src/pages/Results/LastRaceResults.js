...
const LastRaceResults = () => {
      ...
      try {
        sendRequest(
          ...
          (responseDataSet) => {
            ...
            const lastRaceResultArray =
              responseDataSet?.MRData?.RaceTable?.Races;
            console.log("lastRaceResultArray", lastRaceResultArray);
            isMounted &&
              dispatch(fetchLastRaceResultSuccess(lastRaceResultArray));
          }
        );
      } catch (error) {
  ...
  const activeLeaderBoard =
    listCategory === "LeaderBoard" ? `${classes.categoryActive}` : "";
  const activeFastestLap =
    listCategory === "Fastest Lap" ? `${classes.categoryActive}` : "";
        ...
        <section className={classes.lastResultsSection}>
          {isLoading ? (
            ...
            lastRaceResults?.map((result, index) => {
              ...
              return (
                <div
                  className="defaultTransition defaultTransition-5MS"
                  key={index}
                >
                  <div
                    className={classes.circuitInfoWrapper}
                    key={result?.round}
                  >
                    <p className={classes.circuitName}>
                      {result?.raceName || "N/A"}
                    </p>
                    <div className={classes.circuitData}>
                      <span>Laps {result?.Results[0]?.laps || "N/A"}</span>
                      <span>
                        Season {result?.season || "N/A"},Round{" "}
                        {result?.round || "N/A"}
                      </span>
                      <span>
                        {result?.date || "N/A"},
                        {(result?.time).split("Z")[0] || "N/A"}
                      </span>
                    </div>
                  </div>
                  <div className={classes.category}>
                    <p
                      ...
                      className={activeLeaderBoard}
                    >
                      LeaderBoard
                    </p>
                    <p
                      onClick={handleCategoryList}
                      className={activeFastestLap}
                    >
                      Fastest Lap
                    </p>
                  </div>
